def construir_mensajes_modelo(solicitud) -> list[dict[str,str]]:
    ...
    partes=[
        ...
        *[f'- {r}' for r in recuerdos[-30:]],
        f"Persona del usuario: {usuario.get('nombre','Usuario')} · apodo: {usuario.get('apodo','')} · pronombres: {usuario.get('pronombres','')} · {usuario.get('descripcion','')}",
        f"Gustos/preferencias del usuario: {usuario.get('gustos','')}",
        f"Límites indicados por el usuario: {usuario.get('limites','')}",
