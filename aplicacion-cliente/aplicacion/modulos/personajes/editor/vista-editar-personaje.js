...
export function renderizarVistaEditarPersonaje(){
  ...
  const visibilidad=e('select',{name:'visibilidad'},[e('option',{value:'Privado',texto:'Privado'}),e('option',{value:'Local',texto:'Solo este dispositivo'})]);visibilidad.value=personaje.visibilidad||'Privado';
  const musica=e('input',{name:'musicaPreferidaUrl',type:'url',value:personaje.musicaPreferidaUrl||'',placeholder:'https://... (opcional)'});
  ...
  const formulario=e('form',{clase:'formulario-personaje'},[
    ...
    campo('Categoría',categoria),campo('Etiquetas',etiquetas,'Separadas por coma.'),campo('Voz',voz),campo('Visibilidad',visibilidad),campo('Música de fondo predeterminada',musica),
    ...
    campo('Modelo predeterminado para chats nuevos',modelo,'Luego puede cambiarse por conversación sin alterar la identidad.'),campo('Perfil de estilo narrativo',perfil,'Los nombres inspirados sirven como presets nuestros; no son modelos extraídos de otra app.'),
